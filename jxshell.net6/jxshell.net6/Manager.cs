...
namespace jxshell.net6
{
	...
	public class Manager : IDisposable
	{
		...
		public jxshell.net6.wrapper getObjectAsType(object o, object xt)
		{
			Type type;
			type = (!(xt is jxshell.net6.wrapper) ? this.getTypeOrGenericType(xt.ToString()) : (Type)((jxshell.net6.wrapper)xt).wrappedObject);
			object o2 = Convert.ChangeType(o, type);
			return jxshell.net6.wrapper.createWrapper(o2, typeDescriptor.loadFromType(type));
		}
		...
		public void loadManyTypes(string types)
		{
			...
			StringBuilder stringBuilder = new StringBuilder();
			jxshell.net6.typeDescriptor.addUsingsStatements(stringBuilder);
			...
			for (int i = 0; i < (int)strArrays.Length; i++)
			{
				...
				jxshell.net6.typeDescriptor typeDescriptor = new jxshell.net6.typeDescriptor(typeOrGenericType, typeName, false);
			...
			{
				csharplanguage.runScript(stringBuilder.ToString(), jxshell.net6.typeDescriptor.generateInMemory);
			}
			foreach (KeyValuePair<Type, type_1> item in dictionary)
			{
				type_1 value = item.Value;
				Type type = csharplanguage.getCompiledAssembly().GetType(string.Concat("jxshell.net6.", value.staticClass));
				ConstructorInfo constructor = type.GetConstructor(new Type[] { typeof(Type), typeof(jxshell.net6.typeDescriptor) });
		...
		public void loadManyTypes(Type[] types)
		{
			...
			for (int i = 0; i < (int)typeArray.Length; i++)
			{
				...
				{
					jxshell.net6.typeDescriptor typeDescriptor = new jxshell.net6.typeDescriptor(type, jxshell.net6.typeDescriptor.getNameForType(type), false);
					if (!typeDescriptor.isCompiled())
					{
						string staticClass = "";
						string instanceClass = "";
			...
			foreach (KeyValuePair<Type, type_1> item in dictionary)
			{
				...
				ConstructorInfo constructor = type2.GetConstructor(new Type[] { typeof(Type), typeof(jxshell.net6.typeDescriptor) });
				value.td.setCompiledWrapper((wrapperStatic)constructor.Invoke(new object[] { value.t, value.td }));
			}
		}
